...
- итоговые короткие summary по каждому блоку сохраняются в [outputs/reports/hypotheses](outputs/reports/hypotheses).

## Что нужно перед первым запуском

1. Создать Python-окружение и установить зависимости из [requirements.txt](requirements.txt).
2. Скопировать [local_paths.template.json](configs/datasets/local_paths.template.json) в `configs/datasets/local_paths.json`.
...
- либо запускать [run_foundation.py](scripts/run_foundation.py) и [run_all_hypotheses.py](scripts/run_all_hypotheses.py) с аргументами `--data-root` и `--masks-root`.
...
- [run_foundation.py](scripts/run_foundation.py)
- [run_h1_interpretable_features.py](scripts/run_h1_interpretable_features.py)
- [run_h2_mask_effect.py](scripts/run_h2_mask_effect.py)
- [run_h3_noise_regimes.py](scripts/run_h3_noise_regimes.py)
- [run_h4_aggregation.py](scripts/run_h4_aggregation.py)
- [run_h5_deep_vs_handcrafted.py](scripts/run_h5_deep_vs_handcrafted.py)
- [run_all_hypotheses.py](scripts/run_all_hypotheses.py)

Какие скрипты считаются публичными точками входа, а какие внутренними служебными шагами, отдельно описано в [scripts/README.md](scripts/README.md).
...
В [\.gitignore](.gitignore) вынесены:
